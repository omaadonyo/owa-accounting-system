...
<body>
    ...
    <div class="dates">
        <div class="dates-left">
            <p><span class="dates-label">{{ __('Issue Date:') }}</span> {{ $quotation->issue_date->format('d M Y') }}</p>
            @if ($quotation->valid_until)
                <p><span class="dates-label">{{ __('Valid Until:') }}</span> {{ $quotation->valid_until->format('d M Y') }}</p>
            @endif
        </div>
        <div class="dates-right">
            <p class="customer-name">{{ $quotation->customer?->name ?? __('Walk-in Customer') }}</p>
            @if ($quotation->customer?->email)
                <p class="customer-email">{{ $quotation->customer->email }}</p>
            @endif
        </div>
    </div>
    ...
    <div class="totals">
        <p><span class="totals-label">{{ __('Subtotal:') }}</span> UGX {{ number_format($quotation->subtotal, 2) }}</p>
        @if ((float) $quotation->discount_amount > 0)
            <p><span class="totals-label">{{ __('Discount:') }}</span> -UGX {{ number_format($quotation->discount_amount, 2) }}</p>
        @endif
        @if ((float) $quotation->tax_amount > 0)
            <p><span class="totals-label">{{ $quotation->tax_name ?? 'Tax' }} ({{ $quotation->tax_rate ?? 0 }}%):</span> UGX {{ number_format($quotation->tax_amount, 2) }}</p>
        @endif
        <hr class="totals-hr">
        <p class="totals-grand">UGX {{ number_format($quotation->total, 2) }}</p>
    </div>
